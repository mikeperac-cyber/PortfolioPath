...
export class TemplateGenerationProvider implements GenerationProvider {
  ...
  async projectBlueprint(input: Record<string, unknown>, context: GenerationContext) {
    ...
    const weeklyRoadmap = (milestones.length ? milestones : ["Define the scope", "Complete the first documented action", "Review and adapt", "Prepare the final deliverable"]).map((milestone, index) => ({
      ...
      reflectionPrompt: "What unexpected variable changed your approach this week? What data contradicted your initial hypothesis?",
  ...
  async reflectionSupport(input: Record<string, unknown>, context: GenerationContext) {
    ...
    const data: ReflectionSupport = {
      questions: ["What unexpected variable changed your approach this week?", "What data or observation contradicted your initial hypothesis?", "If you had another month, what would you re-test?"],
  ...
  async admissionsExport(input: Record<string, unknown>, context: GenerationContext) {
    const title = text(input, "title", "This project")
    const objective = text(input, "objective", "a documented student objective")
    const hours = Math.max(1, Math.min(10, Number(input.weeklyHours) || 3))
    const weeks = Math.max(1, Math.min(52, Number(input.durationWeeks) || 12))

    const data: AdmissionsExport = {
      commonAppActivity: {
        position: `Founder/Lead, ${title}`.substring(0, 50),
        organization: "Independent Project (PortfolioPath Verified)",
        description: `Objective: ${objective}. Documented evidence of planning, sustained execution, and iterative adaptation. Confirmed by counselor.`.substring(0, 150),
        participationGradeLevels: ["11", "12"],
        timingOfParticipation: "School Year",
        hoursSpentPerWeek: hours,
        weeksSpentPerYear: weeks,
      },
      ucPiqBulletPoints: [
        `Demonstrated initiative by designing and executing ${title}.`,
        `Faced and overcame specific challenges by adapting original plan based on new evidence.`,
        `Developed and practiced verifiable skills relevant to intended major.`
      ],
      academicAbstract: `This independent project, titled "${title}", aimed to ${objective}. Through a structured methodology involving weekly planning, evidence collection, and guided reflection, the project yielded documented outcomes and demonstrated a capacity for sustained intellectual engagement. All primary assertions and skill developments have been subjected to verification.`
    }
    return wrap(data, context, ["This is an AI-synthesized draft for your application. Please review against character limits and ensure it accurately reflects your work."])
  }
}
