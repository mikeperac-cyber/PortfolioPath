...
import { ArrowRightIcon, CalendarDaysIcon, CheckCircle2Icon, Clock3Icon, DownloadIcon, EyeIcon, FileArchiveIcon, LightbulbIcon, LinkIcon, LoaderCircleIcon, LockKeyholeIcon, MessageSquareTextIcon, MoreHorizontalIcon, NotebookPenIcon, PlusIcon, RefreshCwIcon, SaveIcon, SendIcon, ShieldCheckIcon, SparklesIcon, TargetIcon, Trash2Icon, UploadCloudIcon } from "lucide-react"
...
const skillRows = [["Research","confirmed","Field notes · Counselor confirmed"],["Critical thinking","supported","Observation analysis · Evidence supported"],["Writing","target","Selected as a target skill"],["Project management","target","Selected as a target skill"]] as const
const taskRows = [["Complete two observation sessions","Aug 8","In progress"],["Upload annotated field notes","Aug 9","Submitted for review"],["Write weekly reflection","Aug 10","Not started"]] as const
const demoProjectId = "70000000-0000-0000-0000-000000000001"
const demoPortfolioPageId = "90000000-0000-0000-0000-000000000001"
...
function Dashboard({locale,studentName}:{locale:string;studentName:string}) {
  ...
  return <>
    <SectionHeader
      ...
      description="Keep it simple: do one real task, save the evidence, then explain what you learned."
      action={<Button asChild><Link href={`/${locale}/student/planner`}>Continue this week<ArrowRightIcon data-icon="inline-end"/></Link></Button>}
    />
    <Alert className="mb-6 border-accent/60 bg-accent/10">
      <TargetIcon/>
      <AlertTitle>Next best action: finish one observation session</AlertTitle>
      <AlertDescription className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
        <span>Record the date and conditions while you work. You can attach the evidence immediately afterward.</span>
        <Button size="sm" variant="outline" asChild><Link href={`/${locale}/student/planner`}>Open task<ArrowRightIcon data-icon="inline-end"/></Link></Button>
      </AlertDescription>
    </Alert>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <Stat icon={Clock3Icon} label="Tasks due" value="3" note="1 due tomorrow"/>
      <Stat icon={FileArchiveIcon} label="Evidence items" value="8" note="2 awaiting review"/>
      <Stat icon={NotebookPenIcon} label="Reflections" value="3" note="Weekly entry due"/>
      <Stat icon={TargetIcon} label="Confirmed skills" value="1" note="3 remain evidence-linked"/>
    </div>
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_.95fr]">
      <Card className="min-w-0">
        <CardHeader className="border-b">
          <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
            <div className="min-w-0"><Badge variant="secondary">Active project</Badge><CardTitle className="mt-3">Marine Observation Log</CardTitle><CardDescription>Diving-based environmental observation and evidence journal</CardDescription></div>
            <div className="text-right"><span className="text-2xl font-semibold text-primary">42%</span><p className="text-xs text-muted-foreground">overall progress</p></div>
          </div>
          <Progress value={42} className="mt-4" aria-label="Project progress: 42 percent"/>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="mb-5 flex items-center justify-between gap-4"><div><p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Current milestone · Week 3</p><p className="mt-1 font-medium">Identify patterns and document uncertainty</p></div><CalendarDaysIcon className="size-5 shrink-0 text-secondary"/></div>
          <Table><TableHeader><TableRow><TableHead>Task</TableHead><TableHead>Due</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{taskRows.map(row=><TableRow key={row[0]}><TableCell className="font-medium">{row[0]}</TableCell><TableCell>{row[1]}</TableCell><TableCell><StatusBadge>{row[2]}</StatusBadge></TableCell></TableRow>)}</TableBody></Table>
          <Button variant="outline" className="mt-5" asChild><Link href={`/${locale}/student/planner`}>View weekly planner<ArrowRightIcon data-icon="inline-end"/></Link></Button>
        </CardContent>
      </Card>
      <div className="flex flex-col gap-6">
        ...
        <Alert><MessageSquareTextIcon/><AlertTitle>Latest counselor feedback</AlertTitle><AlertDescription>Add the date and observation conditions to your two newest photographs before they are accepted.</AlertDescription></Alert>
      </div>
    </div>
    <div className="mt-6 grid gap-6 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Portfolio readiness</CardTitle><CardDescription>Complete these before private sharing.</CardDescription></CardHeader><CardContent className="flex flex-col gap-4">{[[true,"Project objective supported"],[true,"Evidence selected"],[false,"Final reflection missing"],[false,"Factual accuracy not confirmed"]].map(([done,label])=><div key={String(label)} className="flex items-center gap-3 text-sm">{done?<CheckCircle2Icon className="size-4 text-success" aria-hidden="true"/>:<span className="size-4 rounded-full border" aria-hidden="true"/>}<span>{label}</span></div>)}<Progress value={50} aria-label="Portfolio readiness: 50 percent"/></CardContent></Card>
      <Card><CardHeader><CardTitle>Skills being developed</CardTitle><CardDescription>Evidence and counselor review—not numerical AI scores.</CardDescription></CardHeader><CardContent className="grid gap-3">{skillRows.slice(0,3).map(row=><div key={row[0]} className="flex items-center justify-between gap-3 border p-3"><div><p className="font-medium">{row[0]}</p><p className="mt-1 text-xs text-muted-foreground">{row[2]}</p></div><StatusBadge>{row[1]}</StatusBadge></div>)}</CardContent></Card>
    </div>
  </>
}
...
function Projects({locale}:{locale:string}) { return <><SectionHeader eyebrow="Projects" title="My projects" description="Only work supported by tasks and evidence can move from planned to completed." action={<Button asChild><Link href={`/${locale}/student/projects/new`}><PlusIcon data-icon="inline-start"/>New project</Link></Button>}/><div className="grid gap-5 lg:grid-cols-2"><Card><CardHeader><div className="flex justify-between"><Badge>Active</Badge><MoreHorizontalIcon className="size-5 text-muted-foreground"/></div><CardTitle className="mt-3">Marine Observation Log</CardTitle><CardDescription>Environmental observation · Jun 15–Sep 4</CardDescription></CardHeader><CardContent><Progress value={42}/><div className="mt-4 flex justify-between text-sm"><span>Week 3 of 8</span><span>8 evidence items</span></div></CardContent><CardFooter><Button variant="outline" asChild><Link href={`/${locale}/student/planner`}>Open workspace<ArrowRightIcon data-icon="inline-end"/></Link></Button></CardFooter></Card><Card className="border-dashed"><CardContent className="flex min-h-64 flex-col items-center justify-center text-center"><LightbulbIcon className="size-8 text-accent"/><h2 className="mt-4 font-semibold">Start another direction</h2><p className="mt-2 max-w-sm text-sm text-muted-foreground">Your plan determines how many projects can be created. Complete Portfolio supports up to three.</p><Button className="mt-5" variant="outline" asChild><Link href={`/${locale}/student/project-ideas`}>Explore ideas</Link></Button></CardContent></Card></div></> }

function Planner(){const [tasks,setTasks]=useState(taskRows.map((row,i)=>({title:row[0],due:row[1],status:row[2],done:i===1})));return <><SectionHeader eyebrow="Week 3 of 8" title="Weekly planner" description="Milestone: identify patterns and document uncertainty. Task completion remains separate from counselor review." action={<Button variant="outline" onClick={()=>toast.success("Week submitted for counselor review")}><SendIcon data-icon="inline-start"/>Submit week</Button>}/><Card><CardHeader><CardTitle>Tasks this week</CardTitle><CardDescription>Estimated commitment: 4 hours</CardDescription></CardHeader><CardContent className="space-y-3">{tasks.map((task,index)=><div key={task.title} className="grid items-start gap-4 border p-4 md:grid-cols-[auto_1fr_auto]"><Checkbox checked={task.done} onCheckedChange={checked=>setTasks(tasks.map((v,i)=>i===index?{...v,done:!!checked}:v))} aria-label={`Complete ${task.title}`}/><div><p className="font-medium">{task.title}</p><p className="mt-1 text-xs text-muted-foreground">Due {task.due} · Estimated 60 min · Priority medium</p><Input className="mt-3" placeholder="Obstacle notes or factual context"/></div><StatusBadge>{task.status}</StatusBadge></div>)}<Button variant="outline"><PlusIcon data-icon="inline-start"/>Add task</Button></CardContent></Card><Card className="mt-6"><CardHeader><CardTitle>Weekly reflection prompt</CardTitle><CardDescription>What changed from your plan, and what evidence shows progress?</CardDescription></CardHeader><CardContent><Textarea rows={5} placeholder="Use a concrete example from this week's work…"/></CardContent><CardFooter><Button><SaveIcon data-icon="inline-start"/>Save reflection</Button></CardFooter></Card></>}
...
function Evidence({studentName}:{studentName:string}){
  ...
  async function upload(){
    ...
    setBusy(true)
    try{
      const response=await fetch("/api/evidence/sign-upload",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({projectId:demoProjectId,fileName:file.name,mimeType:file.type,sizeBytes:file.size})})
      const signed=await response.json()
      if(!response.ok)throw new Error(signed.error??"Could not authorize upload")
      const supabase=createClient()
      const {error:uploadError}=await supabase.storage.from("evidence").uploadToSignedUrl(signed.path,signed.token,file,{contentType:file.type})
      if(uploadError)throw uploadError
      const {data:{user}}=await supabase.auth.getUser()
      if(!user)throw new Error("Your session has expired.")
      const extension=file.name.split(".").pop()?.toLowerCase()
      const evidenceType=file.type.startsWith("image/")?"image":file.type.startsWith("video/")?"video":extension==="xlsx"||extension==="csv"?"spreadsheet":"document"
      const {error:recordError}=await supabase.from("evidence").insert({project_id:demoProjectId,student_id:user.id,title:title.trim(),description:explanation.trim(),evidence_type:evidenceType,storage_path:signed.path,mime_type:file.type,size_bytes:file.size,student_explanation:explanation.trim(),review_status:"pending",privacy:"private"})
      if(recordError)throw recordError
      setTitle("");setExplanation("");setFile(null)
      const input=document.getElementById("evidence-file") as HTMLInputElement|null
      if(input)input.value=""
      toast.success("Evidence uploaded privately. Its review status is pending.")
  ...
  return <>
    ...
    <Card>
      ...
      <CardFooter className="justify-end border-t"><Button disabled={busy||!file||!title.trim()||!explanation.trim()} onClick={upload}>{busy?<LoaderCircleIcon data-icon="inline-start" className="animate-spin"/>:<UploadCloudIcon data-icon="inline-start"/>}{busy?"Uploading privately…":"Upload as private evidence"}</Button></CardFooter>
    </Card>
    <Card className="mt-6"><CardHeader><CardTitle>Project evidence</CardTitle><CardDescription>Uploader, date, and review state remain visible as provenance.</CardDescription></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Evidence</TableHead><TableHead>Week</TableHead><TableHead>Skill</TableHead><TableHead>Review</TableHead><TableHead>Privacy</TableHead></TableRow></TableHeader><TableBody>{[["Annotated dive log.pdf","3","Research","Accepted","Private"],["Site conditions.jpg","3","Critical thinking","Clarification requested","Private"],["Observation spreadsheet.xlsx","2","Data analysis","Pending","Private"]].map(row=><TableRow key={row[0]}><TableCell><span className="font-medium">{row[0]}</span><span className="block text-xs text-muted-foreground">Uploaded by {studentName} · Aug 4</span></TableCell><TableCell>{row[1]}</TableCell><TableCell>{row[2]}</TableCell><TableCell><StatusBadge>{row[3]}</StatusBadge></TableCell><TableCell>{row[4]}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
  </>
}

function Reflections(){return <><SectionHeader eyebrow="Student-authored" title="Reflection journal" description="Concrete examples matter more than polished language. Counselors can comment, but reflections receive no automatic score."/><Tabs defaultValue="weekly"><TabsList><TabsTrigger value="weekly">Weekly</TabsTrigger><TabsTrigger value="midpoint">Midpoint</TabsTrigger><TabsTrigger value="final">Final</TabsTrigger></TabsList>{["weekly","midpoint","final"].map(type=><TabsContent key={type} value={type}><Card><CardHeader><CardTitle className="capitalize">{type} reflection</CardTitle><CardDescription>What did you complete? What was difficult? What decision did you make? What evidence shows progress? What will you do next?</CardDescription></CardHeader><CardContent><Textarea rows={10} defaultValue={type==="weekly"?"I completed two short observation sessions. The visibility was lower than expected, so I changed the log to record conditions before interpreting patterns.":""} placeholder="Use concrete examples from your own work…"/></CardContent><CardFooter className="justify-between"><span className="text-xs text-muted-foreground">Clearly labeled as a student-authored statement</span><Button><SaveIcon data-icon="inline-start"/>Save reflection</Button></CardFooter></Card></TabsContent>)}</Tabs></>}

function Skills(){return <><SectionHeader eyebrow="Evidence-linked" title="Skills tracker" description="PortfolioPath uses only three states: target, evidence-supported, and counselor-confirmed."/><Card><CardContent className="pt-6"><Table><TableHeader><TableRow><TableHead>Skill</TableHead><TableHead>Status</TableHead><TableHead>Connected evidence</TableHead><TableHead>Review</TableHead></TableRow></TableHeader><TableBody>{skillRows.map(row=><TableRow key={row[0]}><TableCell className="font-medium">{row[0]}</TableCell><TableCell><StatusBadge>{row[1]}</StatusBadge></TableCell><TableCell>{row[2]}</TableCell><TableCell><Button size="sm" variant="ghost"><EyeIcon/>Open evidence</Button></TableCell></TableRow>)}</TableBody></Table></CardContent></Card></>}

function Feedback(){return <><SectionHeader eyebrow="Counselor review" title="Counselor feedback" description="Feedback cannot modify your source work. You decide how to revise your content."/><div className="space-y-4">{[["Evidence clarification","Add the date, location context, and water conditions to the two newest photos.","Aug 4"],["Reflection comment","Good explanation of why the original comparison was not reliable. Link the updated observation sheet.","Aug 2"]].map(row=><Card key={row[0]}><CardContent className="flex gap-4 pt-6"><MessageSquareTextIcon className="mt-1 size-5 text-secondary"/><div><div className="flex items-center gap-3"><h2 className="font-semibold">{row[0]}</h2><span className="text-xs text-muted-foreground">{row[2]}</span></div><p className="mt-2 text-sm leading-6 text-muted-foreground">{row[1]}</p><Button variant="link" className="mt-2 px-0">Open related record<ArrowRightIcon data-icon="inline-end"/></Button></div></CardContent></Card>)}</div></>}
...
function Portfolio(){const [selected,setSelected]=useState(portfolioSections);const [confirmed,setConfirmed]=useState(false);const [link,setLink]=useState<{id:string;url:string}|null>(null);const [busy,setBusy]=useState(false);async function createLink(){setBusy(true);try{const response=await fetch("/api/share-links",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({portfolioPageId:demoPortfolioPageId,expiresInDays:30})});const body=await response.json();if(!response.ok)throw new Error(body.error??"Could not create link");setLink({id:body.id,url:body.url});toast.success("Private link created for 30 days")}catch(error){toast.error(error instanceof Error?error.message:"Could not create link")}finally{setBusy(false)}}async function revokeLink(){if(!link)return;setBusy(true);try{const response=await fetch("/api/share-links",{method:"DELETE",headers:{"content-type":"application/json"},body:JSON.stringify({id:link.id})});const body=await response.json();if(!response.ok)throw new Error(body.error??"Could not revoke link");setLink(null);toast.success("Private link revoked immediately")}catch(error){toast.error(error instanceof Error?error.message:"Could not revoke link")}finally{setBusy(false)}}return <><SectionHeader eyebrow="Private preview" title="Portfolio builder" description="Arrange, edit, and hide sections. Planned and completed information remains visibly distinct." action={<Button variant="outline"><EyeIcon data-icon="inline-start"/>Preview</Button>}/><div className="grid gap-6 xl:grid-cols-[.75fr_1.25fr]"><Card><CardHeader><CardTitle>Sections</CardTitle><CardDescription>Choose what the private page includes.</CardDescription></CardHeader><CardContent className="space-y-2">{portfolioSections.map(section=><label key={section} className="flex items-center gap-3 border p-3 text-sm"><Checkbox checked={selected.includes(section)} onCheckedChange={checked=>setSelected(checked?[...selected,section]:selected.filter(v=>v!==section))}/>{section}</label>)}</CardContent></Card><Card><CardHeader><Badge variant="outline" className="w-fit">Private preview</Badge><CardTitle className="mt-3 text-3xl">Marine Observation Log</CardTitle><CardDescription>Student role: independent observer and documentarian · Jun–Sep 2026</CardDescription></CardHeader><CardContent className="space-y-7">{selected.map(section=><section key={section} className="border-t pt-5"><h3 className="text-lg font-semibold text-primary">{section}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{section==="Outcomes"?"Planned outcome: complete six documented observation sessions. Current evidence supports three completed sessions.":"Editable guidance assembled from verified project fields and student-authored reflections."}</p></section>)}</CardContent></Card></div><Card className="mt-6"><CardHeader><CardTitle>Export and private sharing</CardTitle><CardDescription>Confirmation is required before publishing, exporting, or creating a link.</CardDescription></CardHeader><CardContent><label className="flex items-start gap-3 text-sm"><Checkbox checked={confirmed} onCheckedChange={value=>setConfirmed(!!value)}/><span>I confirm that this page is factually accurate, distinguishes plans from completed work, and includes no invented observations or impact claims.</span></label>{link&&<Alert className="mt-4"><LinkIcon/><AlertTitle>Private link active</AlertTitle><AlertDescription><a className="break-all underline" href={link.url} target="_blank" rel="noreferrer">{link.url}</a></AlertDescription></Alert>}</CardContent><CardFooter className="flex-wrap gap-3"><Button disabled={!confirmed||busy} onClick={()=>window.open(`/api/portfolio/${demoProjectId}/pdf`,"_blank")}><DownloadIcon data-icon="inline-start"/>Export PDF</Button><Button disabled={!confirmed||busy||!!link} variant="outline" onClick={createLink}><LinkIcon data-icon="inline-start"/>Create 30-day link</Button>{link&&<Button disabled={busy} variant="destructive" onClick={revokeLink}><Trash2Icon data-icon="inline-start"/>Revoke link</Button>}</CardFooter></Card></>}

function GeneratedWorkspace({type}:{type:"presentation"|"recommendation"}){const [busy,setBusy]=useState(false);const [data,setData]=useState<Record<string,unknown>|null>(null);async function generate(){setBusy(true);const response=await fetch(`/api/generate/${type==="presentation"?"presentation":"recommendation-evidence"}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({projectId:demoProjectId,locale:"en",title:"Marine Observation Log",completedTasks:["Completed three observation sessions"],reviewedEvidence:["Annotated field log"],confirmedSkills:["Research"],reflections:["Adapted the log to record visibility conditions"],outcomes:["Three documented sessions"]})});const body=await response.json();if(response.ok)setData(body.data);else toast.error(body.error??"Generation failed");setBusy(false)}return <><SectionHeader eyebrow="Editable guidance" title={type==="presentation"?"Presentation builder":"Recommendation evidence summary"} description={type==="presentation"?"Generate editable 30-second, 90-second, and three-minute versions plus interview preparation.":"Assemble only completed tasks, reviewed evidence, counselor-confirmed skills, reflections, and documented outcomes."} action={<Button onClick={generate} disabled={busy}>{busy?<LoaderCircleIcon data-icon="inline-start" className="animate-spin"/>:<RefreshCwIcon data-icon="inline-start"/>}Generate draft</Button>}/>{type==="recommendation"&&<Alert className="mb-6"><ShieldCheckIcon/><AlertTitle>Required warning</AlertTitle><AlertDescription>This document provides evidence for a recommender. It is not a recommendation letter and must not contain invented observations.</AlertDescription></Alert>}<Card><CardHeader><CardTitle>{data?"Generated draft":"Ready to generate"}</CardTitle><CardDescription>Source IDs and provenance are stored with each generation request. Review factual accuracy before use.</CardDescription></CardHeader><CardContent>{data?<Textarea rows={22} value={JSON.stringify(data,null,2)} onChange={e=>{try{setData(JSON.parse(e.target.value))}catch{}}}/>:<div className="flex min-h-64 flex-col items-center justify-center text-center"><SparklesIcon className="size-8 text-accent"/><p className="mt-4 max-w-lg text-sm text-muted-foreground">The deterministic launch provider assembles content only from supplied source records and never invents counselor observations.</p></div>}</CardContent></Card></>}
