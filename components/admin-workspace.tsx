...
const adminData: Record<string, { title: string; description: string; headers: string[]; rows: string[][] }> = {
  users: { title: "User management", description: "Approve counselors, suspend accounts, and inspect only essential account metadata.", headers: ["User", "Role", "State", "Created", "Action"], rows: [["deniz@demo.local", "Student", "Active", "Aug 1", "Suspend"], ["selin@counselor.local", "Counselor", "Pending", "Aug 4", "Approve"]] },
  assignments: { title: "Counselor assignments", description: "Create and deactivate explicit student–counselor assignments.", headers: ["Student", "Counselor", "State", "Assigned", "Action"], rows: [["Deniz Kaya", "Selin Arslan", "Active", "Aug 1", "Deactivate"]] },
  ...
  templates: { title: "Project templates", description: "Curate bilingual, deterministic project directions with feasibility and ethics metadata.", headers: ["Template", "Category", "Locale", "State", "Action"], rows: [["Marine Observation Log", "Environmental action", "EN / TR", "Active", "Edit"], ["Local Tourism Website", "Coding", "EN / TR", "Active", "Edit"]] },
...
export function AdminWorkspace({ section }: { section: string }) {
  ...
  return <>
    ...
    <Card>
      ...
      <CardContent><Table><TableHeader><TableRow>{data.headers.map((header) => <TableHead key={header}>{header}</TableHead>)}</TableRow></TableHeader><TableBody>{data.rows.map((row, index) => <TableRow key={index}>{row.map((cell, cellIndex) => <TableCell key={cellIndex}>{cellIndex === 2 && ["Active", "Pending", "Open"].includes(cell) ? <Badge variant={cell === "Active" ? "default" : "outline"}>{cell}</Badge> : cell}</TableCell>)}</TableRow>)}</TableBody></Table></CardContent>
