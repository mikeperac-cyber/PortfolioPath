...
export async function POST(request:Request){try{const input=schema.parse(await request.json());return await generateResponse("recommendation_evidence",input,input.sourceRecordIds)}catch(error){return invalid(error)}}
